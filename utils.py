...
#        print("im_end:",im_end)
        ...
        for j in range(n_cap_shard):
            ...
            with torch.no_grad():
                 im = Variable(torch.from_numpy(images[im_start:im_end]), volatile=True).float().to(device)#cuda()
                 s = Variable(torch.from_numpy(captions[cap_start:cap_end]), volatile=True).to(device)#cuda()
                 l = torch.LongTensor(lengths[cap_start:cap_end]).to(device)#cuda()
