...
def train(train_loader, model, optimizer, epoch, opt={}):
    ...
    for i, train_data in enumerate(train_loader):
        ...
        if torch.cuda.is_available():
            input_visual = input_visual.to(device)#cuda()
            input_text = input_text.to(device)#cuda()
            lengths = lengths.to(device)#cuda()

        scores, loss1 = model(input_visual, input_text, lengths, True)
        torch.cuda.synchronize()
        loss = utils.calcul_loss(scores, input_visual.size(0), margin, max_violation=max_violation, )
        #loss = lossf(scores,input_visual.size(0))
