...
def _resnet(arch, block, layers, pretrained, progress, modal, **kwargs):
    ...
    if pretrained:
        ...
        model.load_state_dict(torch.load('layers/resnet18-5c106cde.pth'), strict=False)
    return model

def resnet181(pretrained=True, progress=True, modal='vision',**kwargs):
...
class Location_Net(nn.Module):
    def __init__(self):
        ...
        # fusion ops
        self.conv_av = nn.Conv2d(1024, 1024, kernel_size=1, bias=False)
        
        self.inplanes = 256
        self.feature1 = nn.Sequential(
            conv1x1(self.inplanes, 64),
            nn.BatchNorm2d(64),
    ...
    def forward(self, av):
        ...
        fea1 = self.feature1(av)  # bs*512*7*7
        attn = 2 - self._mask(fea1, av)

        av = av.mul(attn.repeat(1, self.inplanes, 1, 1))
        fea2 = self.feature2(av)
        attn = 2 - self._mask(fea2, av)

        av = av.mul(attn.repeat(1, self.inplanes, 1, 1))
        fea3 = self.feature3(av)
        attn = 2 - self._mask(fea3, av)
        
        av = av.mul(attn.repeat(1, self.inplanes, 1, 1))
        fea4 = self.feature4(av)
        
        av = torch.cat([fea1, fea2, fea3, fea4], dim=1)
